...
export function createDPAIEngine(config: EngineConfig = configFromEnv()): DPAIEngine {
  ...
  if (fastTest) {
    // Local test contract: exactly one OpenAI edit per DP4/DP6, keep the first
    // returned image, skip expensive QA/cross-piece loops, and let the API mark
    // the resulting dossier TEST / NON VALIDE.
