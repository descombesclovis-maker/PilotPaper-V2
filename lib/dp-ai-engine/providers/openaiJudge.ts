...
const schema = {
  ...
  properties: {
    ...
    photorealismScore:{type:["number","null"],minimum:0,maximum:1}, materialRealistic:{type:["boolean","null"]}, lightingMatched:{type:["boolean","null"]}, reflectionsNatural:{type:["boolean","null"]}, contactShadowsNatural:{type:["boolean","null"]}, edgeIntegrationNatural:{type:["boolean","null"]}, localSharpnessMatched:{type:["boolean","null"]}, localNoiseCompressionMatched:{type:["boolean","null"]}, cgiArtifactsAbsent:{type:["boolean","null"]}, roofTexturePreserved:{type:["boolean","null"]},
...
export class OpenAIQualityJudge implements QualityJudge {
  constructor(private apiKey: string, private model = "gpt-5.6-sol", private passScore = 0.96, private realismPassScore = 0.97) {}
  async judge({ dp, form, context, originalPhotos, generated }: Parameters<QualityJudge["judge"]>[0]): Promise<QualityReport> {
    ...
    if(dp===6 && generated.sourceRole){
      ...
      if(base?.mimeType==="image/png" && polys.length){
        try{
          const originalCrop=cropPngAroundPolygons(base.base64,polys,.05);
          const candidateCrop=cropPngAroundPolygons(generated.base64,polys,.05);
          imageDataUrls.push(`data:image/png;base64,${originalCrop}`,`data:image/png;base64,${candidateCrop}`);
        ...
        }catch{/* Full-frame QA still runs if crop cannot be built. */}
      }
    }
    const report = await openaiJson<QualityReport>({
      apiKey:this.apiKey, model:this.model, prompt,
      imageDataUrls, schemaName:"dp_quality_report", schema:schema as unknown as Record<string, unknown>
    });
