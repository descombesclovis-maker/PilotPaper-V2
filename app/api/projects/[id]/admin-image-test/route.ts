...
type ProjectRow = { id: string; siteAddress: string; formData: string };
type FileRow = { kind: string; fileName: string; mimeType: string; sha256: string; objectKey: string };
type TestInput = {
  dp?: 4 | 6;
...
function buildIgnImageUrl(longitude: number, latitude: number, groundWidthMeters: number, groundHeightMeters: number) {
  ...
  const params = {
    ...
    STYLES: "normal,normal", CRS: "EPSG:3857", BBOX: bbox,
    WIDTH: String(IMAGE_WIDTH), HEIGHT: String(IMAGE_HEIGHT), FORMAT: "image/png", TRANSPARENT: "false",
  };
...
async function fetchIgnPng(url: URL) {
  ...
  if (contentType !== "image/png" || bytes.byteLength < 50_000) throw new Error("La vue IGN reçue n'est pas exploitable.");
  return bytes;
}
async function projectCoordinates(address: string, details: Record<string, unknown>) {
  ...
  url.searchParams.set("q", address); url.searchParams.set("index", "address"); url.searchParams.set("limit", "1");
...
function makeForm(projectId: string, address: string, input: TestInput): ProjectForm {
  ...
  if (layoutMode === "fixed" && rows * columns !== count) throw new Error(`Le calepinage fixe ${rows}×${columns} doit contenir exactement ${count} panneaux.`);
  return {
    projectId, address,
    panel: { model: input.moduleReference?.trim() || "Module test", widthMm: positive(input.moduleWidthMm, "Largeur module"), heightMm: positive(input.moduleHeightMm, "Hauteur module"), frameColor: input.frameColor?.trim() || "noir" },
    requestedPanelCount: count,
    array: {
      rows, columns, orientation: input.orientation === "landscape" ? "landscape" : "portrait",
      roofFace: input.roofFace?.trim() || "A",
      placement: input.placement === "left" || input.placement === "right" ? input.placement : "centered",
      layoutMode, allowSplitAcrossFaces: layoutMode === "automatic",
    ...
    roofGeometry: { slopeDeg: Math.max(0, Math.min(75, Number(input.roofPitchDeg ?? 0))), source: "ign-derived" },
    roofSelection: input.roofFace?.trim() ? { mode: "priority", priorityFaceId: input.roofFace.trim() } : { mode: "automatic" },
    support: { topology: input.roofTopology ?? "unknown", covering: input.covering ?? "unknown", existingStructure: true },
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!isPilotPaperAdmin(user.email, configured("PILOTPAPER_ADMIN_EMAILS"))) return Response.json({ error: "Laboratoire réservé aux administrateurs PilotPaper." }, { status: 403 });
  ...
  if (!input || (input.dp !== 4 && input.dp !== 6)) return Response.json({ error: "Sélectionnez DP4 ou DP6." }, { status: 400 });
  ...
  try {
    ...
    const satellite: InputPhoto = { role: "satellite", mimeType: "image/png", base64: base64(satelliteBytes), filename: "ign-satellite.png", widthPx: IMAGE_WIDTH, heightPx: IMAGE_HEIGHT, metersPerPixel: SITUATION_GROUND_WIDTH_METERS / IMAGE_WIDTH };
    const satelliteMass: InputPhoto = { role: "satellite_mass", mimeType: "image/png", base64: base64(massBytes), filename: "ign-satellite-mass.png", widthPx: IMAGE_WIDTH, heightPx: IMAGE_HEIGHT, metersPerPixel: MASS_GROUND_WIDTH_METERS / IMAGE_WIDTH };
    ...
    const editor = new OpenAIImageEditor(apiKey, configured("DP_IMAGE_MODEL") || "gpt-image-2", "projective");
    const renderPhotos = input.dp === 4 ? [roof, near] : [near, roof];
    const asset = await editor.edit({ dp: input.dp, context: contextResult, photos: renderPhotos, prompt: dpImagePrompt(input.dp, contextResult) });
    ...
    const panelPolygons = allPanelPolygonsForRoleProjective(contextResult, asset.sourceRole);
    if (!panelPolygons || panelPolygons.length !== contextResult.exactPanelCount) throw new Error("La projection projective panneau par panneau est incomplète sur l'image source choisie.");

    const deterministicAudit = auditDeterministicImage({ sourceBase64: sourcePhoto.base64, outputBase64: asset.base64, panelPolygons, expectedPanelCount: contextResult.exactPanelCount });
    ...
    const audit = {
      schemaVersion: 2, createdAt, projectId, dp: input.dp, status: "test_unverified",
      projectionModel: "planar-homography-v1",
      sourceRole: asset.sourceRole, sourceFileName: sourcePhoto.filename,
      model: { analysis: configured("DP_ANALYSIS_MODEL") || "gpt-5.6-sol", image: configured("DP_IMAGE_MODEL") || "gpt-image-2" },
      requestedConfiguration: {
        moduleReference: form.panel.model, moduleWidthMm: form.panel.widthMm, moduleHeightMm: form.panel.heightMm,
        frameColor: form.panel.frameColor, moduleCount: form.requestedPanelCount, rows: form.array.rows, columns: form.array.columns,
        orientation: form.array.orientation, layoutMode: form.array.layoutMode, panelGapMm: form.array.interPanelGapMm,
        gutterClearanceMm: form.array.gutterClearanceMm, ridgeClearanceMm: form.array.ridgeClearanceMm,
        placement: form.array.placement, requestedRoofFace: form.array.roofFace, roofPitchDeg: form.roofGeometry?.slopeDeg,
        roofTopology: form.support?.topology, covering: form.support?.covering,
      },
      resolvedGeometry: { roofConfidence: contextResult.roof.confidence, exactPanelCount: contextResult.exactPanelCount, facePlacements: contextResult.facePlacements, immutableFacts: contextResult.immutableFacts, projectedPanelCount: panelPolygons.length },
      officialEvidence: { longitude, latitude, situationUrl: situationUrl.toString(), massUrl: massUrl.toString(), situationMetersPerPixel: SITUATION_GROUND_WIDTH_METERS / IMAGE_WIDTH, massMetersPerPixel: MASS_GROUND_WIDTH_METERS / IMAGE_WIDTH },
      deterministicAudit,
    ...
    await Promise.all([
      env.BUCKET.put(objectKey, bytes, { httpMetadata: { contentType: "image/png" }, customMetadata: { projectId, ownerEmail: user.email, kind: `admin_dp${input.dp}_test`, sha256, status: "test_unverified", sourceRole: asset.sourceRole, projectionModel: "planar-homography-v1", deterministicAuditPassed: String(deterministicAudit.passed), auditObjectKey, createdAt } }),
      env.BUCKET.put(auditObjectKey, auditBytes, { httpMetadata: { contentType: "application/json" }, customMetadata: { projectId, ownerEmail: user.email, kind: `admin_dp${input.dp}_test_audit`, imageSha256: sha256, projectionModel: "planar-homography-v1", createdAt } }),
    ]);
    await env.DB.prepare(
      `INSERT INTO project_files (id, project_id, owner_email, kind, file_name, mime_type, size_bytes, sha256, object_key, status)
    ...
    console.log(`[PilotPaper][ADMIN-LAB] DP${input.dp} projection=homography audit=${deterministicAudit.passed ? "PASS" : "WARN"}; panels=${deterministicAudit.panelCountProjected}/${deterministicAudit.panelCountExpected}; outsideChanged=${deterministicAudit.changedOutsidePixels}; overlaps=${deterministicAudit.overlapPairs}`);
    return new Response(bytes, {
      status: 200,
      headers: {
        "Content-Type": "image/png", "Cache-Control": "private, no-store",
        "X-PilotPaper-Admin-Lab": "true", "X-PilotPaper-DP": String(input.dp),
        "X-PilotPaper-Source-Role": asset.sourceRole, "X-PilotPaper-SHA256": sha256,
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Le laboratoire d'image a échoué." }, { status: 422 });
  }
}
