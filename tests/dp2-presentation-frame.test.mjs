...
const source = await readFile(new URL("../lib/dp2-v1-engine.ts", import.meta.url), "utf8");
...
test("DP2 uses only a small geocoded address-access point instead of a target-parcel highlight", () => {
  ...
  assert.doesNotMatch(source, /PARCELLE \$\{escapeXml\(context\.parcelReference\)\} · \$\{project\.exactPanelCount\} MODULES/);
});

test("panel geometry is reprojected from the metric evidence frame into the wider display frame", () => {
